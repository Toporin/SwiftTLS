...
public struct ECDSA : Signing
{
    public var algorithm: X509.SignatureAlgorithm
    
    var curve : EllipticCurve
    
    public var privateKey : BigInt?
    ...
    public init?(publicKeyInfo : X509.SubjectPublicKeyInfo)
    {
    ...
    public func sign(data : [UInt8]) -> (BigInt, BigInt)
    {
    ...
    public func verify(signature: [UInt8], data: [UInt8]) -> Bool
    {
    ...
    public func verify(signature : (BigInt, BigInt), data: [UInt8]) -> Bool
    {
        ...
        for (section, object) in ASN1Parser.sectionsFromPEMFile(file)
        {
            ...
            case "EC PARAMETERS":
                if let oid = object as? ASN1ObjectIdentifier,
                    ...
                    case .ansip256k1:
                        curve = EllipticCurve.named(.secp256k1)
                    default:
